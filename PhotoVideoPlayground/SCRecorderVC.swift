...
class SCRecorderVC: UIViewController {
    ...
    private var session: SCRecordSession!
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "toVideoRedactor" {
            guard let recorderVC = segue.destination as? SCRedactorVC else { return }
            recorderVC.session = session
        }
    }
    ...
    private func initializeRecorder() {
        ...
        recorder.frameRate = 120

        do {
            let _ = try recorder.prepare()
        } catch {
            print(error)
        }
...
extension SCRecorderVC: SCRecorderDelegate {
